...
contract SunswapExchange is TRC20, ReentrancyGuard {

    uint constant NO_FEE = uint(- 1);  //-1 means no fee
    ...
    ISunswapFactory factory;     // interface for the factory that created this contract
    ...
    event TokenToToken(address indexed buyer, address from_exchangne, address to_exchange, uint256 from_value, uint256 to_value);
    ...
    event AdminFeeMint(address indexed from, address indexed to, uint256 value);
    ...
    function setup(address token_addr) public {
        ...
        factory = ISunswapFactory(msg.sender);
    ...
    function _mintFee(uint256 _reserve0, uint256 _reserve1) private returns (bool feeOn) {
        ...
        uint _kLast = kLast;
        uint feeToRate = ISunswapFactory(factory).feeToRate();
        // gas savings
        if (feeOn) {
            if (_kLast != 0 && feeToRate != NO_FEE) {
    ...
    private returns (uint256)
    {
        ...
        uint256 tokens_bought = ISunswapExchange(exchange_addr).trxToTokenTransferInput.value(wei_bought)(min_tokens_bought, deadline, recipient);
        emit TrxPurchase(buyer, tokens_sold, wei_bought);
        emit Snapshot(buyer, address(this).balance, token.balanceOf(address(this)));
        emit TokenToToken(buyer, address(this), exchange_addr, tokens_sold, tokens_bought);
        return tokens_bought;
    }
    ...
    private returns (uint256)
    {
        ...
        require(exchange_addr != address(this) && exchange_addr != address(0), "illegal exchange addr");
        uint256 trx_bought = ISunswapExchange(exchange_addr).getTrxToTokenOutputPrice(tokens_bought);
        uint256 tokens_sold = getOutputPrice(trx_bought, token.balanceOf(address(this)), address(this).balance);
        // tokens sold is always > 0
        require(max_tokens_sold >= tokens_sold && max_trx_sold >= trx_bought, "max token sold not matched");

        require(address(token).safeTransferFrom(buyer, address(this), tokens_sold), "transfer failed");
        uint256 trx_sold = ISunswapExchange(exchange_addr).trxToTokenTransferOutput.value(trx_bought)(tokens_bought, deadline, recipient);
    ...
    function addLiquidity(uint256 min_liquidity, uint256 max_tokens, uint256 deadline) public payable nonReentrant returns (uint256) {
        ...
        if (total_liquidity > 0) {
            ...
            //jusdge if feeOn
            bool feeOn = _mintFee(trx_reserve, token_reserve);
            ...
            _totalSupply = _totalSupply.add(liquidity_minted);

            require(address(token).safeTransferFrom(msg.sender, address(this), token_amount), "transfer failed");

            uint256 reserve0 = address(this).balance;
            uint256 reserve1 = token.balanceOf(address(this));
            if (feeOn) kLast = uint(reserve0).mul(reserve1);
            // reserve0 and reserve1 are up-to-date

            emit AddLiquidity(msg.sender, msg.value, token_amount);
            emit Snapshot(msg.sender, reserve0, reserve1);
    ...
    function removeLiquidity(uint256 amount, uint256 min_trx, uint256 min_tokens, uint256 deadline) public nonReentrant returns (uint256, uint256) {
        ...
        uint256 trx_amount = amount.mul(trx_reserve) / total_liquidity;
        ...
        emit Snapshot(msg.sender, address(this).balance, reserve1);
        emit Transfer(msg.sender, address(0), amount);

        if (feeOn) kLast = (address(this).balance).mul(reserve1);
        // reserve0 and reserve1 are up-to-date

        return (trx_amount, token_amount);
    }

    function updateKLast() public nonReentrant {
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 trx_reserve = address(this).balance;
        bool feeOn = _mintFee(trx_reserve, token_reserve);

        if (feeOn) kLast = trx_reserve.mul(token_reserve);
    }


    function _mint(address to, uint liquidity_minted) internal {
        _balances[to] = _balances[to].add(liquidity_minted);
        _totalSupply = _totalSupply.add(liquidity_minted);
        emit Transfer(address(0), to, liquidity_minted);
        emit AdminFeeMint(address(this), to, liquidity_minted);
    }
